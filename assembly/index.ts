...
@unmanaged class __arraySegment {
...
@unmanaged class __valueSegment<T> {
  type: AS_SERDE_INSTRUCTION_TYPE;
  size: i32;
  offset: usize;
  value: T;
...
@unmanaged class __circularSegment {
  type: AS_SERDE_INSTRUCTION_TYPE;
  offset: usize;
  isManaged: bool;
  id: u32;
...
@unmanaged class __referenceSegment {
  type: AS_SERDE_INSTRUCTION_TYPE;
  classId: u32;
  offset: usize;
  size: usize;
...
export class Serializer {
  ...
  put<T>(value: T, offset: usize, seen: Map<usize, u32> = new Map<usize, u32>()): void {
    if (isInteger(value) || isFloat(value)) {
      this.writeValue<T>(value, offset);
    }
    ...
    if (seen.has(ptr)) {
      ...
      this.writeArray<T>(value, offset, seen);
    } else {
      this.writeReference<T>(value, offset, seen);
    }
  }
  ...
  writeValue<T>(value: T, offset: usize): void {
    let len = this.length;
    let next = len + offsetof<__valueSegment<T>>();
    this.ensureSize(next);
    let ref = changetype<__valueSegment<T>>(this.ptr + this.length);
